...
/**
 * NavigationProps let individual pages tune the mobile bar without
 * re-wiring every page. For the 90% case, no props are needed.
 */
...
export const Navigation = ({ mobileTitle, mobileShowBack, hideMobileSearchIcon }: NavigationProps = {}) => {
  ...
  const handleSignOut = async () => {
    try {
      ...
      toast({ title: "Error signing out", description: "There was a problem signing you out.", variant: "destructive" });
    }
  };

  return (
    <>
      {/* Mobile shell: top bar + bottom tab nav (always render, CSS hides on md+) */}
      <MobileTopBar
        title={mobileTitle}
        showBack={mobileShowBack}
        hideSearch={hideMobileSearchIcon}
      />
      <MobileBottomNav />

      {/* Desktop / tablet navigation (hidden on mobile) */}
      <nav
        className={`hidden md:block fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'py-2 backdrop-blur-xl shadow-lg shadow-black/5 border-b' : 'py-4'}`}
        style={{
      ...
      >
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between">
              ...
              {navItems.map((item) => (
                ...
                >
                  <Button
                    ...
                    style={{
                      color: isActive(item.path) ? '#D4AF37' : isScrolled ? (isDark ? '#FFFFFF' : '#2D3E50') : '#FFFFFF'
                    }}
                  >
                    ...
                    <span className={`absolute inset-0 rounded-full transition-all duration-300 ${isActive(item.path) ? 'bg-[#D4AF37]/15' : hoveredItem === item.path ? 'bg-[#D4AF37]/10' : 'bg-transparent'}`} />
                    <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-[#D4AF37] rounded-full transition-all duration-300 ${isActive(item.path) ? 'w-6' : 'w-0'}`} />
                  </Button>
                </Link>
              ))}
            </div>
            ...
            <div className="hidden lg:flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Link to={isAdmin ? '/admin-dashboard' : '/user-dashboard'}>
                        <div
                          ...
                          style={{
                            background: isScrolled ? (isDark ? 'rgba(255, 255, 255, 0.1)' : 'rgba(45, 62, 80, 0.1)') : 'rgba(255, 255, 255, 0.15)',
                        ...
                        >
                          <div className="relative">
                            ...
                            {isAdmin && <div className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-green-500 rounded-full border-2" style={{ borderColor: isDark ? '#101820' : '#FFFFFF' }} />}
                          </div>
                          <div className="flex flex-col">
                            <span className="text-sm font-semibold" style={{ color: isScrolled ? (isDark ? '#FFFFFF' : '#2D3E50') : '#FFFFFF' }}>
                              {user.email?.split('@')[0]}
                            </span>
                            <span className="text-xs" style={{ color: isScrolled ? (isDark ? 'rgba(255,255,255,0.6)' : '#666666') : 'rgba(255,255,255,0.7)' }}>
                              {isAdmin ? 'Admin' : 'Member'}
                            </span>
                          </div>
                          <ChevronRight className="w-4 h-4" style={{ color: isScrolled ? (isDark ? 'rgba(255,255,255,0.5)' : '#666666') : 'rgba(255,255,255,0.5)' }} />
                        </div>
                      </Link>
                      <Button
                        variant="ghost"
              ...
              <div className="w-px h-6" style={{ background: isScrolled ? (isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)') : 'rgba(255,255,255,0.2)' }} />
              <ThemeToggle />
              <CartDrawer />
            </div>

            {/* Tablet (md .. lg) compact menu for the desktop bar */}
        ...
        {/* Collapsible tablet menu (desktop bar only) */}
        <div className={`md:block lg:hidden absolute top-full left-0 right-0 transition-all duration-500 ${isMobileMenuOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
          <div
                ...
                {navItems.map((item, index) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {isActive(item.path) && <div className="ml-auto w-2 h-2 bg-[#D4AF37] rounded-full" />}
                    </div>
                  </Link>
                ))}
                    ...
                    {user ? (
                      <div className="space-y-3">
                        <Link to={isAdmin ? '/admin-dashboard' : '/user-dashboard'} onClick={() => setIsMobileMenuOpen(false)}>
                          <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(45, 62, 80, 0.05)' }}>
